...
WaveData getWaveData(vec4 position) {
    ...
    float waveShiftCoefficient = 1.5 * 1.0;
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        float innerWaveComponent;
        float independent;

        #ifdef WAVE_1
            innerWaveComponent = dot(positionVec2, directionTimesFrequency) + tickBySpeed;
            independent = amplitude * frequency * exp(sin(innerWaveComponent) - 1.0) * cos(innerWaveComponent);

            waveValue = amplitude * exp(sin(innerWaveComponent) - 1.0);

            derivative.x = independent * direction.x;
            derivative.y = independent * direction.y;
        #endif

        #ifdef WAVE_2
            float p = 1.5;

            innerWaveComponent = 0.5 * (dot(positionVec2, directionTimesFrequency) + tickBySpeed);
            independent = 0.5 * amplitude * frequency * cos(innerWaveComponent) * sin(innerWaveComponent) * pow(abs(sin(innerWaveComponent)), p - 2.0) * -p;

            waveValue = amplitude * (1.0 - pow(abs(sin(innerWaveComponent)), p));

            if (sin(innerWaveComponent) == 0.0) {
                independent = 0.0;
            }
