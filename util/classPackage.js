...
class Vector3 {
    ...
    static random(min = 0, max = 1) {
        ...
        return new Vector3(Math.random() * maxminusmin + min, Math.random() * maxminusmin + min, Math.random() * maxminusmin + min);
    }
    ...
    reflect(v2) {
        let dotProd = this.dotProd(v2) * 2;
        return this.difference(v2.scaled(dotProd));
    }
    ...
    rotateRad(rads, origin = Vector3.neutral()) {
        ...
        var ca = Math.cos(rads.z);
        var sa = Math.sin(rads.z);
        var cb = Math.cos(rads.y);
        var sb = Math.sin(rads.y)
        var cc = Math.cos(rads.x);
        var sc = Math.sin(rads.x);

        var xx = ca*cb;
        var xy = ca*sb*sc - sa*cc;
        var xz = ca*sb*cc + sa*sc;

        var yx = sa*cb;
        var yy = sa*sb*sc + ca*cc;
        var yz = sa*sb*cc - ca*sc;
